...
export default function RewardsPage() {
  ...
  useEffect(() => {
    const fetchUserDataAndRewards = async () => {
      ...
      setLoading(true)
      try {
        const fetchedTransactions = await getRewardTransactions(userInfo.id)
        setTransactions(fetchedTransactions as Transaction[])
        const fetchedRewards = await getAvailableRewards(userInfo.id)
  ...
  const handleRedeemReward = async (rewardId: number) => {
    if (!userInfo?.id) {
      toast.error('Please connect your wallet to redeem rewards.')
      return
    }
    ...
    if (reward && balance >= reward.cost && reward.cost > 0) {
      try {
        ...
        await createTransaction(userInfo.id, 'redeemed', reward.cost, `Redeemed ${reward.name}`);
  ...
  const handleRedeemAllPoints = async () => {
    if (!userInfo?.id) {
      toast.error('Please connect your wallet to redeem points.');
      return;
    }
    ...
    if (balance > 0) {
      try {
        // Update database
        await redeemReward(userInfo.id, 0);
        
        // Create a new transaction record
        await createTransaction(userInfo.id, 'redeemed', balance, 'Redeemed all points');
