...
// Use NEXT_PUBLIC_GEMINI_API_KEY for client-side
...
export default function CollectPage() {
  ...
  const handleVerify = async () => {
    ...
    if (!geminiApiKey) {
      toast.error('Gemini API key is not set. Please contact support.')
    ...
    try {
      ...
      // Gemini may return JSON in a code block, so strip it if present
      text = text.trim()
      if (text.startsWith('```')) {
